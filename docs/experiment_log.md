...
### 实验 3.b：更换向量模型为 all-mpnet-base-v2

**目的：**  
在保持语料、问题集和评测方式完全一致的前提下，将向量检索模型从 `all-MiniLM-L6-v2` 换成更大的 `all-mpnet-base-v2`，观察性能是否提升。

**设置：**

- 语料：`data/docs.jsonl`（300 篇与感染/抗生素相关的 PubMed 摘要）
- 问题：`data/qa_med_questions.jsonl`（14 个问题：7 英文 + 7 中文）
...
| 向量模型                     | P_micro | R_micro | F1_micro | P_macro | R_macro | F1_macro |
|------------------------------|---------|---------|----------|---------|---------|----------|
| all-MiniLM-L6-v2             | 0.223   | 0.322   | 0.264    | 0.221   | 0.322   | 0.236    |
| all-mpnet-base-v2（本实验）  | 0.210   | 0.244   | 0.226    | 0.161   | 0.219   | 0.169    |

**结论/观察：**

- 换用更大的 `all-mpnet-base-v2` 后，无论 micro 还是 macro，F1 都**低于**原来的 `all-MiniLM-L6-v2`。
- 尤其是召回率（R）从 0.322 降到 0.244，说明在相同 Top-K 下，MPNet 找到的“标准药物”反而更少。
- 在目前这个「小语料 + 药物列表预测」的任务设定下，**大模型并未带来收益**，甚至略有退步。
- 这也从侧面说明：**语料与任务本身的匹配程度，比单纯堆大模型更关键**，为后续「改造语料 / 调整问题集」提供了动机。
